...
def dashboard():
    ...
    try:
        ...
        # Pegar data da última atualização antes de excluir a coluna
        ultima = df_original["Atualizado_em"].max()
        ultima = pd.to_datetime(ultima).strftime("%d-%m-%y %H:%M")
...
def auto_update():
    while True:
        ...
        try:
            ...
            print(f"Atualizando jogos às {datetime.now().strftime('%H:%M:%S')}...")
